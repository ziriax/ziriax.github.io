...
To make things simple, let's start with a **2D real coordinate space,** $\large \mathbb{R}^2$

The elements in this space are called **vectors**, often written like $\large \vec{v}$

Every vector can be represented as a pair of **coordinates** $\large (x,y)$, where $x$ and $y$ are real numbers, so $\large x \in \mathbb{R}$ and $y \in \mathbb{R}$.
...
For every vector $\large \vec{u}, \vec{v}, \vec{w} \in \mathbb{R}^2$
, and $\large \lambda \in \mathbb{R}$, we want this product to have the following axioms:
<br/>
<br/>
...
First of all, every vector $\large\vec{v}$ now has an **inverse**:
...
But using **axiom D**, we can expand $\large\vec{c}^2$ as 
...
> Alternatively, you might conclude that $\large \hat{a}\hat{b} = \hat{b}\hat{a} = 0$.
> <br/>
...
For example, let's square $\large \hat{a}\hat{b}$. 
...
We already know that $\large \hat{a}^2=\hat{b}^2=1$ and $\large\hat{a}\hat{b}=-\hat{b}\hat{a}$, so
...
$$\huge\vec{v_1} \wedge \vec{v_2}$$

<br/>

$\large\vec{v_1} \wedge \vec{v_2}$ is called a **bi-vector**. If you interpret a **vector** as an **oriented line segment**, then a **bi-vector** can be seen as an **oriented plane segment**.
...
$$\huge\vec{v_1} \: \vec{v_2} = \vec{v_1} \cdot \vec{v_2} + \vec{v_1} \wedge \vec{v_2}$$

$$= \Large\|\vec{v_1}\|\|\vec{v_2}\|(\cos\theta + i\:\sin\theta)$$

<br/>

If we now pick **any two orthogonal unit vectors** for $\large\hat{v_1}$ and $\large\hat{v_2}$, we get 

$$\Large\|\hat{v_1}\|=1 \:\:\: \Large\|\hat{v_2}\|=1 \:\:\: \Large\theta=\frac{\pi}{2} \:\:\: \Large\cos\theta = 0 \:\:\: \Large\sin\theta=1$$

<br/>

Using these, our product **simplifies** to

$$\huge\hat{v_1}\:\hat{v_2}= 0 + i\:1 =i $$

<br/>

This means that the geometric product between **any two orthogonal unit vectors** results in $\large i$.
...
*This blog was written with a lot of help of GPT4 at https://chat.openai.com*
